...
{
    oriModel = pangolin::LoadGeometry(modelPath);
    thread = new std::thread(&tVisualizer::run, this);
    return;
}
...
void tVisualizer::draw_line(
    const Eigen::Vector3d& v_min, const Eigen::Vector3d& v_max, 
    float sickness, const Eigen::Vector4d& v_color)
{
    ...
    const pangolin::Geometry& v_geometry)
{
    ...
    default_prog.SetUniform("T_cam_norm", cam.GetModelViewMatrix());
