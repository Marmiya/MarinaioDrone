...
{
public:
	Airsim_tools(const Eigen::Vector3d& v_drone_start) :m_drone_start(v_drone_start)
	{
...
std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> demo_move_to_next(msr::airlib::MultirotorRpcLibClient& vAgent,
	const Eigen::Vector3d& v_next_pos_airsim, float angle, const float v_speed, bool is_forward = true);