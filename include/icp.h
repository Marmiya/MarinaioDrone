...
Eigen::Matrix4d estimate_transform(const PointSet3& v_source_points, const PointSet3& v_target_points,
    uint num_iters, double& avg_dist_ptr, const TreeType1* v_source_tree=nullptr, const TreeType2* v_target_tree=nullptr, float square_distance_threshold = 1e-6f)
{
    ...
    for (uint i_iter = 0; i_iter < num_iters + 1; ++i_iter) {
        ...
        for (int j = 0; j < v_source_points.size(); ++j) {
            ...
            Point3 vertex(vertex4.x()/vertex4.w(),vertex4.y()/vertex4.w(),vertex4.z()/vertex4.w());
            Point3 cp;
        	if(v_target_tree)
...
template <typename TreeType1, typename TreeType2>
void prepare_correspondance(const PointSet3& v_source_points, const PointSet3& v_target_points,
...
template <typename TreeType1, typename TreeType2>
Eigen::Matrix4d estimate_transform_plus(const PointSet3& v_source_points, const PointSet3& v_target_points,
