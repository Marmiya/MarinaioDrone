...
namespace intersectiontools {
	...
	RTCScene generate_embree_scene(const SurfaceMesh& v_mesh)
	{
		...
		struct Vertex { float x, y, z; }; // FIXME: rename to Vertex4f
		struct Triangle { int v0, v1, v2; };

		Vertex* vertices = (Vertex*)rtcSetNewGeometryBuffer(d_mesh, RTC_BUFFER_TYPE_VERTEX, 0, RTC_FORMAT_FLOAT3, sizeof(Vertex), v_mesh.num_vertices());
		Triangle* triangles = (Triangle*)rtcSetNewGeometryBuffer(d_mesh, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3, sizeof(Triangle), v_mesh.num_faces());
		...
		{
			int vertex_index = 0;
			for (const auto& item : v_mesh.vertices_around_face(v_mesh.halfedge(face)))
			{
				if (vertex_index == 0)
				{
		...
		const RTCScene& v_scene, double max_distance, const SurfaceMesh& v_mesh)
	{
		...
		Eigen::Vector3d view_to_point = v_point_pos - v_view_pos;
		double view_to_origin_point = view_to_point.norm();
		view_to_point.normalize();
		//std::cout<<view_to_origin_point;
		if (max_distance != -1.)
			if (max_distance < view_to_origin_point)
				return false;

		// Circle frustum
		if (view_to_point.dot(v_view_direction) < std::cos(v_fov_degree / 180 * M_PI))
			return false;
		...
		if (rayhits.hit.geomID != RTC_INVALID_GEOMETRY_ID) {
			...
			//std::cout<<Eigen::Vector3d(rayhits.ray.tfar * view_to_point + v_view_pos - v_point_pos).norm()<< std::endl;

			auto iter = v_mesh.vertices_around_face(v_mesh.halfedge(*(v_mesh.faces_begin() + rayhits.hit.primID))).begin();
			Point3 p1 = v_mesh.point(*iter);
			iter++;
			Point3 p2 = v_mesh.point(*iter);
			iter++;
			Point3 p3 = v_mesh.point(*iter);
			Vector3 v1 = p2 - p1;
			Vector3 v2 = p3 - p1;
			Point3 hit_position = p1 + (v1 * rayhits.hit.u + v2 * rayhits.hit.v);
			//std::cout << (cgal_point_2_eigen(hit_position) - v_point_pos).norm() << std::endl;
			return rayhits.ray.tfar >= view_to_origin_point || (cgaltools::cgal_point_2_eigen(hit_position) - v_point_pos).norm() <= 1e-3;
		}
		else {
			return false;
		}
		...
		const RTCScene& v_scene, double max_distance, const SurfaceMesh& v_mesh)
	{
		...
		// Horizontal
		Eigen::Vector3d pixel_pos = v_intrinsic_matrix * v_camera_matrix * v_point_pos;
	...
	{
		std::vector<SurfaceMesh> meshes(v_meshes);

		Tree tree;
		for (int i = 0; i < v_meshes.size(); ++i)
		{
			meshes[i].add_property_map<SurfaceMesh::face_index, int>("instance_id", i);
...
#pragma omp parallel for
		for (int y = 0; y < v_height; y++) {
			for (int x = 0; x < v_width; x++) {
				...
				Ray3 ray(Point3(0, 0, 0), Vector3(p.x(), p.y(), p.z()));
				auto result = tree.first_intersection(ray);
				auto result1 = tree.first_intersected_primitive(ray);
				if (result1) {
					const SurfaceMesh::face_index face_index = result1->first;
					const SurfaceMesh* mesh = result1->second;
					if (mesh) {
						const Primitive_id& primitive_id = boost::get<Primitive_id>(result->second);
						for (int id_mesh = 0; id_mesh < meshes.size(); id_mesh++) {
							SurfaceMesh::Property_map<SurfaceMesh::face_index, int> gnus_iter;
							bool found;
							boost::tie(gnus_iter, found) = meshes[id_mesh].property_map<SurfaceMesh::face_index, int>("instance_id");

							SurfaceMesh::Property_map<SurfaceMesh::face_index, int> gnus;
							boost::tie(gnus, found) = mesh->property_map<SurfaceMesh::face_index, int>("instance_id");

							if (gnus_iter[*(meshes[id_mesh].faces_begin())] == gnus[face_index])
								img_instance.at<cv::Vec3b>(y, x)[2] = id_mesh;
						}
						img_distance_perspective.at<float>(y, x) = std::sqrt(CGAL::squared_distance(boost::get<Point3>(result->first), Point3(0, 0, 0)));

						img_distance_planar.at<float>(y, x) = boost::get<Point3>(result->first)[2];
					}
				}
			}
		}
	...
	PointSet3 remove_points_inside(const SurfaceMesh& v_mesh, const std::vector<Point3>& v_points)
	{
		...
		for (int i = 0; i < v_points.size(); i++)
		{
			...
			for (int i_camera = 0; i_camera < camera.size(); ++i_camera) {
				Ray3 ray(v_points[i], camera[i_camera]);
	...
	void remove_points_inside(const SurfaceMesh& v_mesh, PointSet3& v_points)
	{
		...
		for (int i = 0; i < v_points.size(); i++)
		{
			...
			for (int i_camera = 0; i_camera < camera.size(); ++i_camera) {
				...
				Ray3 ray(v_points.point(i), camera[i_camera]);
				if (int result = tree.number_of_intersected_primitives(ray) <= 1)
				{
					visible_flag += 1;
				}
			}
		...
		v_points.collect_garbage();
		std::cout << v_points.size() << std::endl;
		
	}

	bool is_visible(const Eigen::Vector3d& v_view_pos, const Eigen::Vector3d& v_view_direction,
		const Eigen::Vector3d& v_point_pos, const Tree& v_tree, const float fov_in_degree_h, const float fov_in_degree_v, float max_distance)
	{
		const Point3 point_pos(v_point_pos.x(), v_point_pos.y(), v_point_pos.z());
		const Point3 view_pos(v_view_pos.x(), v_view_pos.y(), v_view_pos.z());
		Vector3 view_direction(v_view_direction.x(), v_view_direction.y(), v_view_direction.z());

		Vector3 view_to_point = point_pos - view_pos;
		Vector3 view_to_point_norm = view_to_point / std::sqrt(view_to_point.squared_length());
		double squared_distance = view_to_point.squared_length();
		if (max_distance != -1.f)
			if (max_distance * max_distance < squared_distance)
				return false;

		// Horizontal
		Vector3 normal_1 = CGAL::cross_product(view_direction, Vector3(0.f, 0.f, 1.f));
		normal_1 /= std::sqrt(normal_1.squared_length());
		double h_delta = std::asin(normal_1 * view_to_point_norm) / 3.1415926f * 180.f;
		//if (h_delta>180.f)
		//	h_delta = 360.f-h_delta;
		h_delta = std::abs(h_delta);

		Vector3 normal_2 = CGAL::cross_product(normal_1, view_direction);
		normal_2 /= std::sqrt(normal_2.squared_length());
		double v_delta = std::abs(std::asin(normal_2 * view_to_point_norm) / 3.1415926f * 180.f);
		v_delta = std::abs(v_delta);
		...
		// Intersection Test
		Ray3 ray_query(view_pos, view_to_point_norm);
		...
		if (intersection && boost::get<Point3>(&(intersection->first))) {
			const Point3* p = boost::get<Point3>(&(intersection->first));
			squared_ray_distance = CGAL::squared_distance(view_pos, *p);
			intersection_point_squared_distance = CGAL::squared_distance(point_pos, *p);
		}
		else
			return false;

		return squared_ray_distance <= squared_distance || intersection_point_squared_distance <= 1e-8;
		//return true;
	}


	bool is_visible(const Eigen::Matrix3d& v_intrinsic_matrix, const Eigen::Isometry3d& v_camera_matrix, const Eigen::Vector3d& v_view_pos, const Eigen::Vector3d& v_point_pos,
		const Tree& v_tree, float max_distance)
	{
		...
		if (max_distance != -1.f)
			if (max_distance * max_distance < view_to_origin_point)
				return false;
		...
		if (intersection && boost::get<Point3>(&(intersection->first))) {
			...
			return false;
		return view_to_intersect_point >= view_to_origin_point || intersect_point_to_origin_point <= 1e-8;
	}
}
